const fetchData = (nodes, edges, graph, that) => {
	...
	return new Promise((resolve, reject) => {
		const p1 = fetch('http://localhost:1323/pod');
		const p2 = fetch('http://localhost:1323/service');
		const p3 = fetch('http://localhost:1323/ingress');
		const p4 = fetch('http://localhost:1323/daemonset')
		...
		.then((values) => {
			...
			.then((objectValues) => {
				...
				for (let i = 0; i < podObject.items.length; i+=1) { 
					...
					newPodObject.label = podObject.items[i].metadata.name; // pod display title
